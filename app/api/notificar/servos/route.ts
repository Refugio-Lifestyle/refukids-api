...
export const OpenAPINotificarTodosServos: RouteConfig = {
    tags: ['Notificar'],
    summary: 'Envia uma notificação para todos os servos',
    method: 'post',
    path: '/notificar/servos',
    security: [{ BearerAuth: [] }],
    request: {
        body: {
            required: true,
    ...
    responses: generateOpenAPIErrorsResponses(z.object(), {
        ...
        '500': ['Falha ao enviar a notificação para todos os servos']
    })
}
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        return Response.json({ error: 'Falha ao enviar a notificação para todos os servos' }, { status: 500 })
    }
