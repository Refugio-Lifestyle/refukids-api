...
export const OpenAPINotificarTodosResponsaveis: RouteConfig = {
    tags: ['Notificar'],
    summary: 'Envia uma notificação para todos os responsáveis',
    method: 'post',
    path: '/notificar/responsaveis',
    security: [{ BearerAuth: [] }],
    request: {
        body: {
            required: true,
    ...
    responses: generateOpenAPIErrorsResponses(z.object(), {
        ...
        '500': ['Falha ao enviar a notificação para todos os responsáveis']
    })
}
...
export async function POST(req: NextRequest) {
    ...
    try {
        const usuariosNotificados = await prisma.responsavel
            .findMany()
        ...
        return Response.json({ error: 'Falha ao enviar a notificação para todos os responsáveis' }, { status: 500 })
    }
